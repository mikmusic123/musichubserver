...
router.post("/complete", upload.fields([
    ...
    try {
        ...
        const vocalsUrl = `/files/jobs/${encodeURIComponent(safeName(jobId))}/${encodeURIComponent(path.basename(vocalsPath))}`;
        const instrumentalUrl = `/files/jobs/${encodeURIComponent(safeName(jobId))}/${encodeURIComponent(path.basename(instPath))}`;
        return res.json({
            ok: true,
...
//# sourceMappingURL=splitter.worker.js.map