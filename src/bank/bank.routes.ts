...
import { EARN_RULES, STORE, type EarnAction, type StoreItemId } from "./bank.jobs.js";
import type { LedgerTx, Wallet } from "./bank.types.js";
...
function hasClientEventId(ledger: LedgerTx[], clientEventId: string) {
  return ledger.some((tx) => tx.clientEventId === clientEventId);
}

function lastTxByReason(ledger: LedgerTx[], reason: string, type: "earn" | "spend") {
  for (let i = ledger.length - 1; i >= 0; i--) {
    const tx = ledger[i]!;
    if (tx.type === type && tx.reason === reason) return tx;
  }
  return null;
}
...
router.post("/wallet/earn", requireAuth, (req: AuthedRequest, res) => {
  const userId = (req as AuthedRequest).user!.id;

  const action = req.body?.action as EarnAction | undefined;
  const clientEventId = req.body?.clientEventId as string | undefined;
  const meta = (req.body?.meta ?? {}) as Record<string, unknown>;
  ...
  try {
    const next = updateWallet(userId, (w:Wallet) => {
      // 1) idempotency
      if (hasClientEventId(w.ledger, clientEventId)) return w;
...
router.post("/wallet/spend", requireAuth, (req: AuthedRequest, res) => {
  const userId = (req as AuthedRequest).user!.id;

  const itemId = req.body?.itemId as StoreItemId | undefined;
  const clientEventId = req.body?.clientEventId as string | undefined;
  const meta = (req.body?.meta ?? {}) as Record<string, unknown>;
