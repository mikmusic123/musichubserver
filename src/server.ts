...
export type Section = {
  ...
  video?: string; // URL (e.g. YouTube, Vimeo, etc.)
};
...
export type ResourceFile = {
  label: string;
  url: string; // could be a file path or remote URL
};
...
export type User = {
  ...
  providerId?: string;         // e.g. Facebook user id / Apple sub
};
...
function writeResources(resources: Resource[]) {
  fs.writeFileSync(
    RESOURCES_PATH,
...
app.post("/auth/login", async (req: Request, res: Response) => {
  try {
    ...
    const user = users.find(
      (u) => u.email.toLowerCase() === email!!.toLowerCase() && u.provider === "local"
    );
...
app.post("/auth/facebook", (req: Request, res: Response) => {
  try {
    const { facebookId, email, name } = req.body as {
      facebookId?: string;
      email?: string;
      name?: string;
    };
    ...
    let user = users.find(
      (u) => u.provider === "facebook" && u.providerId === facebookId
    );
...
app.post("/auth/apple", (req: Request, res: Response) => {
  try {
    ...
    let user = users.find(
      (u) => u.provider === "apple" && u.providerId === appleId
    );
