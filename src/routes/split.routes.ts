...
async function createWorkerJob(file: Express.Multer.File) {
  ...
  // buffer -> blob (TS-safe)
const part = file.buffer as unknown as any; // ✅ silence BlobPart typing mess

const blob = new Blob([part], {
  type: file.mimetype || "application/octet-stream",
});

form.append("file", blob as any, file.originalname);
...
async function fetchWorkerJob(jobId: string) {
  ...
  if (!res.ok) {
    const body = await readBodySafe(res);
    throw new Error(
      `Worker status failed (${res.status} ${res.statusText}): ${body}`
    );
  }
